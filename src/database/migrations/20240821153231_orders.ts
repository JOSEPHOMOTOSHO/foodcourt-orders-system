...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('orders', (table) => {
    ...
    table.integer('calculated_order_id').unsigned().references('id').inTable('calculated_orders').onDelete('SET NULL').nullable();
    ...
    table.jsonb('order_total_amount_history').nullable();
    ...
    table.integer('order_type_id').unsigned().notNullable().references('id').inTable('order_types').onDelete('SET NULL');
  });
}
